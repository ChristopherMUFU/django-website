...
const Paiement = (props) => {
  ...
  const estValide = (fieldValues = state) => {
    ...
    if (location.delivery === undefined)
      validator.delivery = "Veuillez choisir un mode de livraison dans le panier";
    ...
    try {
      ...
      if (payload.error) {
        ...
        baskets.forEach((item) => {
          ...
          if (item.type === "menu") {
            menus.push({ menu_produits: item.produits, quantite: item.quantite, menu_type: item.nom });
          } else {
            produits.push({
              produit_id: item.id,
        ...
        console.log('les menus et produits :: ' + JSON.stringify(menus) + ', ' + JSON.stringify(produits));
        ...
        console.log('les data envoyées :: ' + data)
        console.log(data);

        sendrequest("post", "paiement/create-commande", null, null, data)
        .then ((response) => {      
          if (response !==false){            
            reinitialiserState();
        ...
        {paiment_process === "livraison" ? (
          ...
          >
            ...
            <div className="delivery-infos__container">
              <p className="paiement__delivery-informations">Choix de livraison : {typeOfDelivery}</p>
